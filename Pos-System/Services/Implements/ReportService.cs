...
namespace Pos_System.API.Services.Implements
{
    public class ReportService : BaseService<ReportService>, IReportService
    {
        ...
        public async Task<ICollection<GetStoreEndDayReport>> GetStoresInBrandEndDayReport(string? storeCode, string? brandCode, DateTime? startDate,
            DateTime? endDate)
        {
            ...
            ICollection<GetStoreEndDayReport> reportList = null;

            if(storeCode != null)
            {
                Store store = await _unitOfWork.GetRepository<Store>().SingleOrDefaultAsync(predicate: x => x.BrandId.Equals(brand.Id));
                var result = GetEndDateReportFunction(brand.Id, store.Id, startDate, endDate).Result;
                reportList.Add(result);
            }
            else
            {
                ICollection<Store> storesInBrand = await _unitOfWork.GetRepository<Store>().GetListAsync(
                    predicate: x => x.BrandId.Equals(brand.Id) && x.Status.Equals(StoreStatus.Active.GetDescriptionFromEnum())
                    );

                foreach(Store store in storesInBrand)
                {
                    var result = GetEndDateReportFunction(brand.Id, store.Id, startDate, endDate).Result;
                    reportList.Add(result);
                }
        ...
        private async Task<GetStoreEndDayReport> GetEndDateReportFunction(Guid brandId, Guid storeId, DateTime? startDate,
            DateTime? endDate)
        {
            GetStoreEndDayReport report = new GetStoreEndDayReport();
